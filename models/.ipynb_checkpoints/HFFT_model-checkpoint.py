...
class HFFT(nn.Module):
    def __init__(self, window_size=[28, 14, 7], dims=[64, 128, 256],
        ...
        self.attn_GCIA = nn.ModuleList([CrossAttention(dims[i], num_heads[i],window_size[i]) for i in range(self.nfs)])
    ...
    def forward(self, x):
        ...
        gcia_outs = [self.attn_GCIA[i](q_gcips[i], q_lms[i], ) for i in range(self.nfs)]
        ffn_gcia_outs = [self.ffn_GCIA[i](gcia_outs[i], gcip_outs[i]) for i in range(self.nfs)]
        ffn_gcia_reverses = [reverse_gcia(ffn_gcia_outs[i], ffn_gcip_outs[i]) for i in range(self.nfs)] 

        outs = [_to_channel_first(o) for o in ffn_gcia_reverses]
