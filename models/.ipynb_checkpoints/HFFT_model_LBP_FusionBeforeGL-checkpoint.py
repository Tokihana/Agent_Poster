...
def _channel_first_to_q(x):
    B, C, H, W = x.shape
    return x.permute(0, 2, 3, 1).reshape(B, H*W, C)

def _channel_last_to_q(x):
    B, H, W, C = x.shape
...
class HFFT_LBP(nn.Module):
    def __init__(self, window_size=[28, 14, 7], dims=[64, 128, 256],
        ...
        # enhancement fusion
        self.lbp_ext = LBP_Extractor(dims)
    ...
    def forward(self, x):
        ...
        lbps = self.lbp_ext(x)
        # shape of extracted features: (B, C, H, W)
        # query needed: (B, H*W, C)
        
        # EHCA
        q_irs = [_channel_first_to_q(ir) for ir in irs]
        q_lbps = [_channel_first_to_q(lbp) for lbp in lbps]
        ehca_outs = [self.attn_EHCA[i](q_irs[i], q_lbps[i]) for i in range(self.nfs)]
        ffn_ehca_outs = [self.ffn_EHCA[i](ehca_outs[i], irs[i].permute(0, 2, 3, 1)) for i in range(self.nfs)]
        
        # GCIP
        # split irs to windows, then use the original shortcut as query
        window_splits = [list(self.windows[i](ffn_ehca_outs[i].permute(0, 3, 1, 2))) for i in range(self.nfs)]
        ...
        q_lms = [_channel_first_to_q(lm) for lm in lms] 
